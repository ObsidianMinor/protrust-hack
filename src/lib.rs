...
//!
//!  * `no_json`: Removes generated json message implementations from generated output, 
//!  similar to the CODE_SIZE optimization.
//!
...
//! --rust_opt=crate_name=foo,no_json,no_reflection,size_checks,external_modules=external_lib::gen+another_lib::generated
//! ```
//!
//! ### Messages
//!
//! The compiler generates messages as structs. They derive the `Clone`, `PartialEq`, `Default`, and `Debug` traits.
//! All messages implement the `CodedMessage` and `LiteMessage` traits. If the message is not in a
//! LITE_RUNTIME optimized file, a `Message` implementation is generated for it as well (unless the
//! `no_reflection` option is used in protoc). If the message defines extension ranges, an
//! implementation for `ExtensionMessage` is also provided.
//!
//! Nested messages, oneof field enums, and extension fields defined in the scope of a message are generated

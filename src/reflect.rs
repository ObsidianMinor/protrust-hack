...
impl FileDescriptor {
    ...
    fn new(proto: *const FileDescriptorProto, pool: &mut DescriptorPool) -> *mut FileDescriptor {
        ...
        descriptor.dependencies = descriptor.proto().dependency
            .iter()
            .map(|f| pool.get_file_ref(f))
            .collect::<Vec<_>>()
            .into();

        descriptor.public_dependencies = descriptor.proto().public_dependency
            .iter()
            .map(|f| Ref::clone(&descriptor.dependencies[*f as usize]))
            .collect::<Vec<_>>()
            .into();

        descriptor.messages = descriptor.proto().message_type
            .iter()
            .map(|m| MessageDescriptor::new(&**m as *const DescriptorProto, CompositeScope::File(Ref::new(descriptor_raw)), pool))
            .collect::<Vec<_>>()
            .into();

        descriptor.enums = descriptor.proto().enum_type
            .iter()
            .map(|e| EnumDescriptor::new(&**e as *const EnumDescriptorProto, CompositeScope::File(Ref::new(descriptor_raw)), pool))
            .collect::<Vec<_>>()
            .into();

        descriptor.services = descriptor.proto().service
            .iter()
            .map(|s| ServiceDescriptor::new(&**s as *const ServiceDescriptorProto, Ref::new(descriptor_raw), pool))
            .collect::<Vec<_>>()
            .into();

        descriptor.extensions = descriptor.proto().extension
            .iter()
            .map(|e| FieldDescriptor::new(&**e as *const FieldDescriptorProto, FieldScope::File(Ref::new(descriptor_raw)), pool))
            .collect::<Vec<_>>()
            .into();
        ...
        if let Some(_) = pool.symbols.insert(descriptor.name().clone(), Symbol::File(descriptor_raw)) {
            panic!()
        }
...
impl MessageDescriptor {
    ...
    fn new(proto: *const DescriptorProto, scope: CompositeScope, pool: &mut DescriptorPool) -> Ref<MessageDescriptor> {
        ...
        descriptor.messages = descriptor.proto().nested_type
            .iter()
            .map(|m| MessageDescriptor::new(&**m as *const DescriptorProto, CompositeScope::Message(Ref::new(descriptor_raw)), pool))
            .collect::<Vec<_>>()
            .into();

        descriptor.enums = descriptor.proto().enum_type
            .iter()
            .map(|e| EnumDescriptor::new(&**e as *const EnumDescriptorProto, CompositeScope::Message(Ref::new(descriptor_raw)), pool))
            .collect::<Vec<_>>()
            .into();

        descriptor.extensions = descriptor.proto().extension
            .iter()
            .map(|e| FieldDescriptor::new(&**e as *const FieldDescriptorProto, FieldScope::Message(Ref::new(descriptor_raw)), pool))
            .collect::<Vec<_>>()
            .into();

        descriptor.oneofs = descriptor.proto().oneof_decl // oneofs before fields since we reference them when determining field scopes
            .iter()
            .map(|o| OneofDescriptor::new(&**o as *const OneofDescriptorProto, Ref::new(descriptor_raw), pool))
            .collect::<Vec<_>>()
            .into();

        descriptor.fields = descriptor.proto().field
            .iter()
            .map(|f| FieldDescriptor::new(&**f as *const FieldDescriptorProto, 
        ...
        let mut number_order = descriptor.fields()
            .iter()
            .map(Ref::clone)
            .collect::<Vec<_>>();
        ...
        if let Some(_) = pool.symbols.insert(descriptor.full_name().clone(), Symbol::Message(descriptor_raw)) {
            panic!()
        }
...
impl EnumDescriptor {
    ...
    fn new(proto: *const EnumDescriptorProto, scope: CompositeScope, pool: &mut DescriptorPool) -> Ref<EnumDescriptor> {
        ...
        descriptor.values = descriptor.proto().value
            .iter()
            .map(|v| EnumValueDescriptor::new(&**v as *const EnumValueDescriptorProto, Ref::new(descriptor_raw), pool))
            .collect::<Vec<_>>()
            .into();

        if let Some(_) = pool.symbols.insert(descriptor.full_name().clone(), Symbol::Enum(descriptor_raw)) {
            panic!()
        }
...
impl EnumValueDescriptor {
    ...
    fn new(proto: *const EnumValueDescriptorProto, parent: Ref<EnumDescriptor>, pool: &mut DescriptorPool) -> Ref<EnumValueDescriptor> {
        ...
        descriptor.full_name = format!("{}.{}", descriptor.enum_type().full_name().clone(), descriptor.name());

        if let Some(_) = pool.symbols.insert(descriptor.full_name().clone(), Symbol::EnumValue(descriptor_raw)) {
            panic!()
        }
...
impl ServiceDescriptor {
    ...
    fn new(proto: *const ServiceDescriptorProto, file: Ref<FileDescriptor>, pool: &mut DescriptorPool) -> Ref<ServiceDescriptor> {
        ...
        descriptor.methods = descriptor.proto().method
            .iter()
            .map(|m| MethodDescriptor::new(&**m as *const MethodDescriptorProto, Ref::new(descriptor_raw), pool))
            .collect::<Vec<_>>()
            .into();

        if let Some(_) = pool.symbols.insert(descriptor.full_name().clone(), Symbol::Service(descriptor_raw)) {
            panic!()
        }
...
impl MethodDescriptor {
    ...
    fn new(proto: *const MethodDescriptorProto, service: Ref<ServiceDescriptor>, pool: &mut DescriptorPool) -> Ref<MethodDescriptor> {
        ...
        descriptor.full_name = format!(".{}.{}", descriptor.service().full_name(), descriptor.name());

        if let Some(_) = pool.symbols.insert(descriptor.full_name().clone(), Symbol::Method(descriptor_raw)) {
            panic!()
        }
...
impl FieldDescriptor {
    ...
    fn new(proto: *const FieldDescriptorProto, scope: FieldScope, pool: &mut DescriptorPool) -> Ref<FieldDescriptor> {
        ...
        if let Some(_) = pool.symbols.insert(descriptor.full_name().clone(), Symbol::Field(descriptor_raw)) {
            panic!()
        }
    ...
    fn cross_ref(&mut self, pool: &mut DescriptorPool) {
        ...
        self.value_type = match self.proto().r#type.unwrap().unwrap() {
            Message => FieldType::Message(pool.get_message_ref(self.proto().type_name.as_ref().unwrap())),
            Enum => FieldType::Enum(pool.get_enum_ref(self.proto().type_name.as_ref().unwrap())),
            Group => FieldType::Group(pool.get_message_ref(self.proto().type_name.as_ref().unwrap())),
        ...
        if let Some(default) = self.proto().default_value.as_ref() {
            self.default = 
                match self.field_type() {
                    FieldType::Enum(e) => DefaultValue::Enum(pool.get_enum_value_ref(&(e.full_name().clone() + "." + default))),
...
impl OneofDescriptor {
    ...
    fn new(proto: *const OneofDescriptorProto, message: Ref<MessageDescriptor>, pool: &mut DescriptorPool) -> Ref<OneofDescriptor> {
        ...
        if let Some(_) = pool.symbols.insert(descriptor.full_name().clone(), Symbol::Oneof(descriptor_raw)) {
            panic!()
        }
